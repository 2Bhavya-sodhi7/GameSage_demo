...
@st.cache_data
def load_data():
    sponsor_df = pd.read_csv("final_sponsor_detections_GT.csv")
    audio_df = pd.read_csv("gt_peak_sponsor.csv")
    df1 = pd.read_csv("ball_by_ball_gt.csv")
    df2 = pd.read_csv("IPL2k24_tweets_data.csv")
    df3 = pd.read_csv("stadium_boundaries.csv")
    df4 = pd.read_csv("dc_engagement_peaks.csv")
...
# 6. Page routing
# =========================================================
df1['runs'] = df1['runs'].astype(str).str.replace('W', '0')
...
""")
        ...
        with col6:
            ...
            peak_counts = exploded.value_counts().drop("NO GT SPONSOR DETECTED", errors="ignore")
...
""")
        ...
        st.write("")
    with st.container():
        with st.expander("From Match 60 DC VS GT 2025 BALL BY BALL DATASET (Power of Prediction and Analyis)", expanded=True):
            st.markdown("""
...
""")
    ...
    ax1.set_xlabel("Shot Direction (2nd Innings GT Batting)")
...
""")
    ...
    with st.container():
        with st.expander("Ball-by-Ball Data (Match 60 DC VS GT 2025)", expanded=True):
            st.markdown("""
...
# Display the second image in the second column
    ...
    with st.container():
        with st.expander("We made a system that can spot good sportsmanship moments in cricket, like handshakes or helping another player. These moments are useful for sponsors to show their ads in a positive light.", expanded=True):
            ...
            image1 = Image.open("image_13_4df34146-6cec-4d6d-8e16-a1dc420c534b.jpg")
            image2 = Image.open("image_181_28ca8ef0-d845-431d-a395-e98c126990d7.jpg")        
            col1, col2 = st.columns(2)
...
# Display the second image in the second column
            ...
            image4 = Image.open("image_68_a5cd2889-c913-43a7-affd-e66da180b337.jpg")        
            col1, col2 = st.columns(2)
...
# Display the second image in the second column
    ...
    with st.container():
        with st.expander("We used an existing system to detect the most exciting moments in cricket match videos by analyzing crowd sounds.", expanded=True):
            ...
            st.video("dc_final_video.mp4")
            
            st.caption("Extracted Audio From The IPL Videos")

            audio_file = open('dc_audio_final.mp3', 'rb')
...
# Mark peaks on the graph
            ...
            image7 = Image.open("gt_labels.jpg")
