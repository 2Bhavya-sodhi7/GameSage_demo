...
# Try loading data once so it’s available if any section uses it
try:
...
if selected == "Home":
    ...
    with st.container():
        with st.expander("Power of Prediction & Analysis", expanded=False):
            st.markdown(
                """
    ...
    with st.container():
        with st.expander("From Twitter(Physical Benchmark)", expanded=False):
            st.markdown("""
...
""")
    ...
    with st.container():
        with st.expander("From Audio Peak Dataset (Missed Branding Opportunities)", expanded=False):
            st.markdown("""
...
""")
    ...
    with st.container():
        with st.expander("From Sponsor Detection Dataset (Missed Branding Opportunities)", expanded=False):
            st.markdown("""
...
""")
    ...
    with st.container():
        with st.expander("From RCB VS PBKS(FINAL) BALL BY BALL DATASET (Power of Prediction and Analyis)", expanded=False):
            st.markdown("""
...
# Use apply(pd.Series) to safely expand coords into x and y columns
    ...
    # Use sponsor_df, audio_df, etc., here as needed
    col_text, col_map = st.columns([1, 2])
    with col_text:
        with st.expander("IPL 2024 Tweets Dataset", expanded=False):
            st.markdown(
                """
